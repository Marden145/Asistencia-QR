...
const ProductoForm = ({producto,onSubmit,onCancelar}) =>{
const { register, handleSubmit, reset, formState: { errors } } = useForm();
useEffect(() => {
    if (producto) {
      reset(producto);
    } else {
      reset({ nombre: '', descripcion: '', cantidad: '', precio: '' });
    }
  }, [producto, reset]);
      ...
      >
        ...
        <div className="flex items-center justify-between mb-8">
          <div>
            ...
            <p className="text-xs mt-0.5" style={{ color: 'rgba(147,197,253,0.4)' }}>
              {producto ? 'Modifica los datos del registro' : 'Completa los datos del nuevo registro'}
            </p>
          </div>
          <motion.button
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <Field label="Nombre"   name="nombre"   required placeholder="Arroz" />
            <Field label="Descripción" name="descripcion" required placeholder="Arroz blanco" />
          </div>
          <Field label="Stock"    name="cantidad"    type="number" placeholder="100" />
          <Field label="Precio"   name="precio"   type="number" placeholder="1.99" />
